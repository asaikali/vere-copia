...
public abstract class AbstractRecordPrinter<T> implements RecordPrinter<T>
{
	...
	{
		return generateRecordsReport(Arrays.asList(rec));
	}
	
	public String generateRecordsReport(Collection<T> recs)
	{
		final var report = generateHeader();
		
		for (T record : recs)
			report.append(generateRecordReportInternal(record).toString()).append("\r\n");
	...
	{
		System.out.print(generateRecordsReport(records));
	}
	
	protected StringBuilder generateRecordReportInternal(T record)
	{
	...
	protected StringBuilder generateHeader()
	{
		StringBuilder builder = new StringBuilder();
